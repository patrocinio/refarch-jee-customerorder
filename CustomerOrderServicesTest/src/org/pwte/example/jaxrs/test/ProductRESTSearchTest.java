...
public class ProductRESTSearchTest extends TestCase {
	...
	public void testProductListByCategory()  {
		RestClient client = new RestClient();
		Resource resource = client.resource(urlPrefix
				+ "jaxrs/Product?categoryId=1");

		JSONArray productList = resource.accept("application/json").get(
				JSONArray.class);
		for (int i = 0; i < productList.size(); i++) {
			...
			case 1: {
				...
				assertEquals("images/Return.jpg", product.get("image"));
				assertEquals(
						"Episode 6, Luke has the final confrontation with his father!",
						product.get("description"));
				break;
			}
			case 2: {
				...
				assertEquals(
						"Episode 5, Luke finds out a secret that will change his destiny",
						product.get("description"));
				break;
			}
			case 3: {
				...
				assertEquals(
						"Episode 4, after years of oppression, a band of rebels fight for freedom",
						product.get("description"));
				break;
			}
			default: {
	...
	public void testCategoryResource() {
		...
		JSONObject category = resource.accept("application/json").get(
				JSONObject.class);
	...
	public void testCategoryListResources()  {
		...
		JSONArray categories = resource.accept("application/json").get(
				JSONArray.class);
		for (int k = 0; k < categories.size(); k++) {
			...
			case 1: {
				assertEquals("Entertainment", category.get("name"));
				assertEquals(new Long(1), category.get("id"));
				JSONArray subCategories = (JSONArray)category
						.get("subCategories");
				for (int i = 0; i < subCategories.size(); i++) {
					JSONObject subCategory = (JSONObject)subCategories.get(i);
					switch (((Long)subCategory.get("id")).intValue()) {
